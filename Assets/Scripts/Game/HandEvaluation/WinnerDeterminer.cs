...
public static class WinnerDeterminer
{
    ...
    public static List<PokerPlayerModel> DetermineWinners(List<PokerPlayerModel> players, List<CardDataSO> communityCards)
    {
        var activePlayers = players.Where(p => !p.IsFolded).ToList();
        var handValues = activePlayers.ToDictionary(
            p => p,
            p => HandEvaluator.EvaluateHand(p.HoleCards, communityCards));

        var bestHand = handValues.Values.OrderByDescending(v => v, Comparer<HandValue>.Create(HandEvaluator.CompareHands)).First();
        var winners = handValues.Where(kvp => HandEvaluator.CompareHands(kvp.Value, bestHand) == 0)
                                .Select(kvp => kvp.Key)
                                .ToList();
